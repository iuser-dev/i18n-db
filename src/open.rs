...
use anyhow::Result;
use rocksdb::{BlockBasedOptions, Cache, DBCompactionStyle, DBCompressionType, Options, DB};

use crate::CF_LI;

pub fn open(path: impl Into<PathBuf>) -> Result<DB> {
  ...
  */
  ...
  let mut db: DB = DB::open_cf(&opt, &path, &cf_li)?;
